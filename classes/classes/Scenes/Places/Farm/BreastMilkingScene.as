package classes.Scenes.Places.Farm 
{
	...
	public class BreastMilkingScene extends AbstractFarmContent 
	{
		...
		public function getMilked():void {
			...
			clearOutput();
	//outputText("<b>Milking:</b>\n");
	//outputText("<i>LactationQ: " + player.lactationQ() + " </i>\n");
	//outputText("<i>Player's lactation endurance: " + player.statusEffectv1(StatusEffects.LactationEndurance) + "</i>\n");
	//var counter:Number = player.breastRows.length;
	//while (counter > 0) {
	//
		//outputText("<i>Breast row " + counter + ":</i>\n");
		//counter--;
		//outputText("<i>   size " + player.breastRows[counter].breastRating + " multiplier " + player.breastRows[counter].lactationMultiplier + "</i>\n");
	//}
	...
	if (rand(4) == 0) player.growTits(0.1, 1, true, 3);
	else player.boostLactation(.05);
	//Reset 'feeder' status
