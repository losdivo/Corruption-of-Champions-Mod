package classes.Scenes.Dungeons.MinoTown 
{
	 ...
	 * @author ...
	 */
	public class KingMinosScene extends BaseContent
	{
		
		public function KingMinosScene()
		{
			super();
		}
		...
		{
			clearOutput();
			outputText("You enter a huge hall. A minotaur like none you’ve ever seen sits on the throne. He stands up and walks down from a throne.");
			...
			outputText("\n\nTaking the gleaming metal shaft from his smaller slave, the proud beast stamps the butt-end into the worked stone floor.");
		...
		public function theKingIsDeadLongLiveTheKing(hpVictory:Boolean):void
		{
			...
			outputText("\n\n You have a semi-conscious royal and his cow-slut to use as you will. Or you could just put them out of their misery.");
			if (player.cor <= 33)
			{
		...
		private function murderhobo():void
		{
			...
			clearOutput();
			outputText("You make it quick. You’ll purge every single ounce of corruption from this world by any means necessary.");
		...
		private function dockucocku(cockIdx:int):void
		{
			...
			if (!player.hasKnot(cockIdx) || player.cocks[cockIdx].knotMultiplier * player.cocks[cockIdx].cockThickness <= 2)
			{
				...
				outputText("\n\nNeither of them will be standing in your way any time soon.");
				combat.cleanupAfterCombat(getGame().dungeons.resumeFromFight);
			}
			else
			{
		...
		private function buttufucku():void
		{
			...
			outputText("\n\nYou kick the minotaur King’s legs apart for better access and sidle in close, one hand idly pumping at " + player.cockDescript() + " while you do your damnedest not to give in to the delicious scent. No matter how pernicious the desire to indulge in his aroma, how wonderful it would be to press your cheek against that pillar of masculinity and lick, you know you have to resist it or you’ll forget all about getting yourself off.");
			outputText("\n\nYou rub your " + player.cockHead(x) + " against King’s entrance, slowly splitting his derriere, reshaping it into a vessel for your pleasure. He’s tight, tighter than you would expect from such a massive creature. His asshole clutches at every inch of cock you feed it, almost like it wants to squeeze you off, to massage you with rapt enthusiasm.");
			...
			outputText(" You erupt a split-second before he does, squirting thick ropes of jism into his sucking asshole. Then he goes off, shooting a torrent of spunk twenty feet into the air.");
			...
			outputText("\n\nThe beaten brute squirms and writhes long after you both of you finish cumming, lost in barely understood pleasure. You pull out before his over-eager motions carry you with him, noisily licking his residue from your lips and preparing to get on with your business. The taste might be keeping you hard, but you feel more than sated enough.");
			player.orgasm('Dick');
			dynStats("lus",10);
			combat.cleanupAfterCombat(getGame().dungeons.resumeFromFight);
		}	
		...
		private function mechanicalbullhue():void
		{
			...
			outputText(". The artificial euphoria this has brought you makes it difficult not to crawl over to the nearest minotaur and offer your body up.");
		...
		private function titfuckCowslut():void
		{
			...
			clearOutput();
			outputText("Forget that beefcake; there’s a primo slut close at hand with a set of tits that’d be worth damnation to fuck. Pulling [oneCock] out of your [armor], you advance on the fallen slut, feeling momentarily sorry for her.");
		...
		private function sloppySeconds():void
		{
			...
			outputText(", or you might have had second thoughts. Her creampied honeypot ought to feel like teflon-treated silk against your [cock " + x + "]. It glitters in the light of throne room, beckoning you to plunge inside and see just how good it will feel.");
			...
			outputText("\n\nFuck, you could love almost anyone right now. You knead her ass some more, watching it give beneath your fingers. Your digits nearly disappear into the cushy, fuckable asscheeks, and you grunt in approval. It’s hypnotic. You’re still fucking her, of course. You couldn’t stop if you wanted to, but there’s a strange disconnect between your hips and the rest of your awareness, like there’s so much pleasure that you’ve gone with numb it.");
			...
			outputText("\n\nYou need to get off more than you ever thought possible. How could you possibly want this to end? You’ve grown so addicted to this pleasure in such a short time that ever leaving it seems an alien concept to your cunt-obsessed mind. You want to fuck this pussy <i>forever.</i>");
