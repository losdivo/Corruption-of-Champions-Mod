package classes.Items
{
	...
	public final class Mutations extends MutationsHelper
	{
		...
		public function minotaurCum(purified:Boolean, player:Player, bottled:Boolean):void
		{
            ...
            else outputText("  The heady taste fills your mouth as you convulsively swallow.");
			//-Raises lust by 10.
			//-Raises sensitivity
			...
			//(Healing вЂ“ if hurt and uber-addicted (hasperk))
		...
		public function clovis(player:Player):void {
			...
			if (rand(3) == 0 && changes < changeLimit) {
				...
				outputText("\n\nYou feel a wave of stubborn pride wash over you as you finish the potion. YouвЂ™re sure nothing could stop you now, not even the demons.");
				changes++;
			}
...
//9)  Transformation Item - Snake Oil (S. Oil)
		...
		public function regularHummus(player:Player):void
		{
			...
			//-Skin color change вЂ“ tan, olive, dark, light
		...
		public function whiskerFruit(player:Player):void
		{
			...
			if (player.countCocksOfType(CockTypesEnum.CAT) > 0 && rand(3) == 0 && changes < changeLimit && !flags[kFLAGS.HYPER_HAPPY]) {
				...
				if (temp == 1) {
					...
					outputText("  Although the package is smaller, it feels even more sensitive вЂ“ as if it retained all sensation of its larger size in its smaller form.");
					dynStats("sen", 5);
		...
		public function reptilum(player:Player):void
		{
			...
			if (player.countCocksOfType(CockTypesEnum.LIZARD) == 0 && player.cockTotal() > 0 && changes < changeLimit && rand(4) == 0) {
				...
				if (player.hasSheath()) {
					...
					if (!player.hasSheath()) outputText("\n\nYour sheath tightens and starts to smooth out, revealing ever greater amounts of your " + player.cockDescript(temp2) + "'s lower portions.  After a few moments <b>your groin is no longer so animalistic вЂ“ the sheath is gone.</b>");
				}
			...
			//-Scales вЂ“ color changes to red, green, white, blue, or black.  Rarely: purple or silver.
		...
		public function neonPinkEgg(pregnantChange:Boolean,player:Player):void
		{
			...
			if (!pregnantChange) outputText("You eat the neon pink egg, and to your delight it tastes sweet, like candy.  In seconds you've gobbled down the entire thing, and you lick your fingers clean before you realize you ate the shell вЂ“ and it still tasted like candy.");
			...
			if (changes < changeLimit && rand(5) == 0) {
				...
				//Libido over 60? FUCK YEAH!
					...
					outputText(".  ");
					if (player.gender == 1) outputText("Compression tightens down on " + player.sMultiCockDesc() + " as it strains against your " + player.armorName + ".  You struggle to fight down your heightened libido, but it's hard вЂ“ so very hard.");
					...
					else if (player.gender == 2) outputText("Moisture grows between your rapidly-engorging vulva, making you squish and squirm as you try to fight down your heightened libido, but it's hard вЂ“ so very hard.");
			...
			if ((player.balls > 0 || player.hasCock()) && player.cumQ() < 3000 && rand(3) == 0 && changeLimit > 1) {
				...
				if (player.balls > 0) {
					...
					if (player.cumQ() < 50) outputText("\n\nA twinge of discomfort runs through your " + player.ballsDescriptLight() + ", but quickly vanishes.  You heft your orbs but they haven't changed in size вЂ“ they just feel a little bit denser.");
					...
					//(XXL cum quantity)
						...
						else outputText("You pant and groan but the pleasure just turns to pain.  You're so backed up вЂ“ if only you had some way to vent all your seed!");
					}
				}
		...
		public function goldenSeed(type:Number,player:Player):void
		{
			...
			if (rand(4) == 0 && player.hasGills() && changes < changeLimit) updateGills();
			//SPECIAL:
			//Harpy Womb вЂ“ All eggs are automatically upgraded to large, requires legs + tail to be harpy.
...
//Mouseover script: \"The green-tinted, hardly corporeal substance flows like a liquid inside its container. It makes you feel...uncomfortable, as you observe it.\"
		...
		public function foxTF(enhanced:Boolean,player:Player):void
		{
			...
			//[Adjust hips toward 10 вЂ“ wide/curvy/flared]
			...
			//[Adjust hair length toward range of 16-26 вЂ“ very long to ass-length]
		...
		public function trapOil(player:Player):void
		{
			...
			//Thigh Loss: (towards вЂњgirlyвЂќ)
			...
			//Thigh Gain: (towards вЂњgirlyвЂќ)
			...
			if (player.balls > 0 && player.hasCock() && (player.ballSize > 1 || !player.hasStatusEffect(StatusEffects.Uniball)) && rand(4) == 0 && changes < changeLimit) {
				...
				if (player.ballSize < 1 && !player.hasStatusEffect(StatusEffects.Uniball)) {
					...
					//[Note: Balls description should no longer say вЂњswings heavily beneathвЂќ.  For simplicity's sake sex scenes should continue to assume two balls]
					player.ballSize = 1;
...
//maybe some pregnancy-accelerating thing
		...
		public function ferretFruit(player:Player):void
		{
			...
			if (player.ferretScore() >= 6 && player.findPerk(PerkLib.TransformationResistance) < 0)
			{
				...
				{
					//-If you fail to heed the warning, itвЂ™s game over:
					outputText("\n\nAs you down the fruit, you begin to feel all warm and fuzzy inside.  You flop over on your back, eagerly removing your clothes.  You laugh giddily, wanting nothing more than to roll about happily in the grass.  Finally finished, you attempt to get up, but something feels...  different.  Try as you may, you find yourself completely unable to stand upright for a long period of time.  You only manage to move about comfortably on all fours.  Your body now resembles that of a regular ferret.  That canвЂ™t be good!  As you attempt to comprehend your situation, you find yourself less and less able to focus on the problem.  Your attention eventually drifts to a rabbit in the distance.  You lick your lips. Nevermind that, you have warrens to raid!");
					getGame().gameOver();
					return;
				}
			}
			...
			//- If butt rating is greater than вЂњpetiteвЂќ:
			...
			if (player.hasCock())
			{
				...
				if (temp >= 0 && rand(2) == 0 && changes < changeLimit)
				{
					if (player.cocks[temp].cockLength > 6 && !flags[kFLAGS.HYPER_HAPPY])
					{
						...
						{
							outputText("  Luckily, it doesnвЂ™t seem to have lost its previous thickness.");
						}
						else
						{
			...
			{
				outputText("\n\nYou scream in agony as a horrible pain racks the entire length of your snake-like coils.  Unable to take it anymore, you pass out.  When you wake up, youвЂ™re shocked to find that you no longer have the lower body of a snake.  Instead, you have soft, furry legs that resemble that of a ferretвЂ™s.  <b>You now have ferret legs!</b>");
				changes++;
			...
			if (player.lowerBody != LOWER_BODY_TYPE_FERRET && player.earType == EARS_FERRET && player.tailType == TAIL_TYPE_FERRET && rand(4) == 0 && changes < changeLimit)
			{
				...
				if (player.isTaur()) outputText("\n\nYou scream in agony as a horrible pain racks your entire horse lower half.  Unable to take it anymore, you pass out.  When you wake up, youвЂ™re shocked to find that you no longer have the lower body of a horse.  Instead, you have soft, furry legs that resemble that of a ferretвЂ™s.  <b>You now have ferret legs!</b>");
			...
			{
				outputText("\n\nYou squint as you feel a change in your ears.  Inspecting your reflection in a nearby puddle you find that <b>your ears have become small, fuzzy, and rounded, just like a ferretвЂ™s!</b>");
			...
			if (changes == 0)
			{
				outputText("\n\nYour eyes widen.  With the consumption of the fruit, you feel much more energetic.  YouвЂ™re wide awake now!");
				changes++;
				player.changeFatigue(-10);
			}
			player.refillHunger(20);
		...
		public function pigTruffle(boar:Boolean, player:Player):void
		{
			...
			if (rand(3) == 0 && changes < changeLimit && player.balls > 0 && player.ballSize < 4) {
				if (player.ballSize < 3)
					outputText("\n\nA flash of warmth passes through you and a sudden weight develops in your groin. You pause to examine the changes and your roving fingers discover your " + (player.balls == 4 ? "quartette" : "duo") + " of [balls] have grown larger than a humanвЂ™s.");
				else
			...
			if (rand(4) == 0 && changes < changeLimit && player.cocks.length > 0 && player.cocks[0].cockType != CockTypesEnum.PIG) {
				if (player.cocks.length == 1) { //Single cock
					outputText("\n\nYou feel an uncomfortable pinching sensation in your [cock]. " + player.clothedOrNakedLower("You pull open your [armor]", "You look down at your exposed groin") + ", watching as it warps and changes. As the transformation completes, youвЂ™re left with a shiny, pinkish red pecker ending in a prominent corkscrew at the tip. <b>You now have a pig penis!</b>");
					player.cocks[0].cockType = CockTypesEnum.PIG;
				}
				else { //Multiple cocks
					outputText("\n\nYou feel an uncomfortable pinching sensation in one of your cocks. You pull open your [armor], watching as it warps and changes. As the transformation completes, youвЂ™re left with a shiny pinkish red pecker ending in a prominent corkscrew at the tip. <b>You now have a pig penis!</b>");
			...
			if (rand(boar ? 3 : 4) == 0 && changes < changeLimit && player.earType != EARS_PIG) {
				outputText("\n\nYou feel a pressure on your ears as they begin to reshape. Once the changes finish, you flick them about experimentally, <b>and youвЂ™re left with pointed, floppy pig ears.</b>");
			...
			if (rand(boar ? 3 : 4) == 0 && changes < changeLimit && player.earType == EARS_PIG && player.tailType == TAIL_TYPE_PIG && player.lowerBody != LOWER_BODY_TYPE_CLOVEN_HOOFED) {
				if (player.isTaur()) //Centaur
					outputText("\n\nYou scream in agony as a horrible pain racks your entire bestial lower half. Unable to take it anymore, you pass out. When you wake up, youвЂ™re shocked to find that you no longer have the animal's lower body. Instead, you only have two legs. They are digitigrade and end in cloven hooves. <b>You now have pig legs!</b>");
				else if (player.lowerBody == LOWER_BODY_TYPE_NAGA) //Naga
					outputText("\n\nYou scream in agony as a horrible pain racks the entire length of your snake-like coils. Unable to take it anymore, you pass out. When you wake up, youвЂ™re shocked to find that you no longer have the lower body of a snake. Instead, you only have two legs. They are digitigrade and end in cloven hooves. <b>You now have pig legs!</b>");
				else //Bipedal
			...
			if (rand(3) == 0 && changes < changeLimit && player.earType == EARS_PIG && player.tailType == TAIL_TYPE_PIG && player.lowerBody == LOWER_BODY_TYPE_CLOVEN_HOOFED && player.faceType == FACE_PIG) {
				outputText("\n\nYou cry out in pain as the bones in your face begin to break and rearrange. You rub your face furiously in an attempt to ease the pain, but to no avail. Your bottom teeth ache as well. WhatвЂ™s happening to you? As the sensations pass, you examine your face in a nearby puddle. <b>You nearly gasp in shock at the sight of your new tusky boar face!</b>");
			...
			if (rand(boar ? 3 : 4) == 0 && changes < changeLimit) {
				...
				outputText("\n\nYour skin tingles ever so slightly as you skinвЂ™s color changes before your eyes. As the tingling diminishes, you find that your skin has turned " + skinToBeChosen + ".");
